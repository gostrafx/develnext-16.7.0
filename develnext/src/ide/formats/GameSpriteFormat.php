...
class GameSpriteFormat extends AbstractFormat
{
    ...
    {
        $dialog = new InputMessageBoxForm('Creating a new sprite', 'Enter a name for the new sprite', '* Only Latin letters, numbers and _');
        $dialog->setResult($name);
        $dialog->setPattern(new Regex('^[a-z\\_]{1}[a-z0-9\\_]{0,60}$', 'i'), 'This name is incorrect');
